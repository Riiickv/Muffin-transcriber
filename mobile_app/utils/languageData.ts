// The language table and the Whisper-code mapping - PURE (no i18n import), so
// it's unit-testable in plain Node. The UI label builders that need translation
// stay in languages.ts and re-export from here.
//
...
export type Language = { name: string; code: string; endonym: string };
...
export const FORMAT_LANGUAGE_VALUES = ['Auto-Detect / Original', ...LANGUAGES.map((l) => l.name)];
